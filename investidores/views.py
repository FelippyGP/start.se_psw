...
def sugestao(request):
    ...
    if request.method == "GET":
        ...
        if tipo == 'C' : 
            ...
            empresas = Empresas.objects.filter(tempo_existencia_in=['-6','+6','+1']).exclude(estagio="E")
        
        empresas = empresas.filter(area_in=area)
        ...
        return render(request, 'sugestao.html',{'empreas': empresas_selecionadas, 'areas': areas})
...
def realizar_proposta(request, id):
    ...
    if total + int(percentual) > empresa.percentual_equity:
        messages.add_message(request, constants.WARNING, f'O percentual solicitado ultrapassa o percentual maximo')

    valuation = (100 * int(valor)) /int(percentual)
    if valuation <(int(empresa.valuation /2 )):
          messages.add_message(request, constants.WARNING, f'Seu valuation foi R${{valuation}} e deve ser no minimo {{empresa.valuation}}')
...
def assinar_contrato(request, id):
    ...
    if request.method == "GET":
        ...
        messages.add_message(request, constants.SUCCESS, f'Contrato assinado com sucesso, sua proposta foi enviado a empresa.')
