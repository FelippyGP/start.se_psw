...
def cadastrar_empresa(request):
    if request.method == "GET":
        ...
        return render(request, 'cadastrar_empresa.html',{'tempo_existencia': Empresas.tempo_existente_choices,'areas': Empresas.area_choices})
        ...
        publico_alvo = request.POST.get('publica_alvo')
...
def add_doc(request, id):
    ...
    if not arquivo:
        messages.add_message(request(request, constants.ERROR, "Envie um arquivo"))
        return redirect(f"/empresarios/empresa/{empresa.id}")
    ...
    return redirect(f'/empresa.html/empresa/{empresa.id}')


def excluir_doc(request, id):
    documento = Documento.obejcts.get(id = id)
